...
App.Pages.Customers = (function () {
  ...
  function addEventListeners() {
    ...
    $customers.on("click", ".customer-row", (event) => {
      ...
      const customer = filterResults.find(
        (filterResult) => Number(filterResult.id) === Number(customerId)
      );
  ...
  function display(customer) {
    ...
    customer.appointments.forEach((appointment) => {
      ...
      if (
        ...
        vars("secretary_providers").indexOf(appointment.id_users_provider) ===
          -1
      ) {
        return;
      }
      ...
      const message = `Bonjour, ${customer.first_name} ${customer.last_name}. Ceci est un rappel de votre rendez-vous du: ${start}  Au ${end} pour le service : ${appointment.service.name}`;
      ...
      $("<div/>", {
        ...
        html: [
          ...
          $("<a/>", {
            href: App.Utils.Url.siteUrl(
              `calendar/reschedule/${appointment.hash}`
            ),
            html: [
              $("<i/>", {
          ...
          $("<a/>", {
            ...
            class: "btn btn-success mt-2", // Style Bootstrap pour le bouton
            html: [
              $("<i/>", {
                class: "fab fa-whatsapp me-2", // Icône WhatsApp
              }),
              "Notifier",
            ],
          }),
    ...
    App.Http.Customers.search(keyword, filterLimit).then((response) => {
      ...
      response.forEach((customer) => {
        $("#filter-customers .results")
          .append(App.Pages.Customers.getFilterHtml(customer))
          .append($("<hr/>"));
      });
  ...
  function select(id, show = false) {
    ...
    if (show) {
      const customer = filterResults.find(
        (filterResult) => Number(filterResult.id) === Number(id)
      );
