  ...
  function display(customer) {
    $id.val(customer.id),
      ...
      customer.appointments.length ||
        $("<p/>", { text: lang("no_records_found") }).appendTo(
      ...
      customer.appointments.forEach(function (appointment) {
        if (
          ...
          (vars("role_slug") !== App.Layouts.Backend.DB_SLUG_SECRETARY ||
            -1 !==
              vars("secretary_providers").indexOf(
            ...
            html: [
              $("<a/>", {
                href: App.Utils.Url.siteUrl(
                  "calendar/reschedule/".concat(appointment.hash)
                ),
                html: [
    ...
    App.Http.Customers.search(keyword, filterLimit).then(function (response) {
      (filterResults = response),
        ...
        response.length
          ...
          : $filterCustomers
              .find(".results")
              .append($("<em/>", { text: lang("no_records_found") })),
        selectId && App.Pages.Customers.select(selectId, show);
    });
  }
  function select(id) {
    var show =
    ...
    if (
      ...
      var customer = filterResults.find(function (filterResult) {
        return +filterResult.id === +id;
      });
      App.Pages.Customers.display(customer),
        $("#edit-customer, #delete-customer").prop("disabled", !1);
    }
  }
    ...
    }),
    {
      ...
      save: function save(customer) {
        App.Http.Customers.save(customer).then(function (response) {
          App.Layouts.Backend.displayNotification(lang("customer_saved")),
            ...
            App.Pages.Customers.filter("", response.id, !0);
        });
      },
      ...
      validate: function validate() {
        ...
        try {
          ...
          if (email && !App.Utils.Validation.email(email))
            throw (
              ($email.addClass("is-invalid"), new Error(lang("invalid_email")))
            );
          var phoneNumber = $phoneNumber.val();
          if (phoneNumber && !App.Utils.Validation.phone(phoneNumber))
            throw (
              ($phoneNumber.addClass("is-invalid"),
              new Error(lang("invalid_phone")))
            );
          return !0;
        } catch (error) {
          return (
            $formMessage.addClass("alert-danger").text(error.message).show(), !1
          );
        }
      },
      getFilterHtml: function getFilterHtml(customer) {
        var name =
            (customer.first_name || "[No First Name]") +
            " " +
            (customer.last_name || "[No Last Name]"),
          info = customer.email || "[No Email]";
        return (
      ...
      resetForm: function resetForm() {
        $customers
          ...
          .prop("disabled", !0),
          $customers
            .find(".record-details .form-label span")
            .prop("hidden", !0),
          $customers
            .find(".record-details #timezone")
            .val(vars("default_timezone")),
          $customers
            .find(".record-details #language")
            .val(vars("default_language")),
          $customerAppointments.empty(),
          $customers
            .find("#edit-customer, #delete-customer")
            .prop("disabled", !0),
          $customers.find("#add-edit-delete-group").show(),
          $customers.find("#save-cancel-group").hide(),
          $customers
            .find(".record-details .is-invalid")
            .removeClass("is-invalid"),
          $customers.find(".record-details #form-message").hide(),
          $filterCustomers.find("button").prop("disabled", !1),
      ...
      addEventListeners: function addEventListeners() {
        $customers.on("submit", "#filter-customers form", function (event) {
          ...
          $customers.on("click", ".customer-row", function (event) {
            if (!$filterCustomers.find(".filter").prop("disabled")) {
              var customerId = $(event.currentTarget).attr("data-id"),
                customer = filterResults.find(function (filterResult) {
                  return +filterResult.id === +customerId;
                });
              display(customer),
                $("#filter-customers .selected").removeClass("selected"),
          ...
          $customers.on("click", "#add-customer", function () {
            App.Pages.Customers.resetForm(),
              ...
              $customers
                .find(".record-details .form-label span")
                .prop("hidden", !1),
              $filterCustomers.find("button").prop("disabled", !0),
          ...
          $customers.on("click", "#cancel-customer", function () {
            var id = $id.val();
            App.Pages.Customers.resetForm(), id && select(id, !0);
          }),
          $customers.on("click", "#save-customer", function () {
            ...
            $id.val() && (customer.id = $id.val()),
              App.Pages.Customers.validate() &&
          ...
          $customers.on("click", "#delete-customer", function () {
            var customerId = $id.val(),
              buttons = [
                {
                  text: lang("cancel"),
                  click: function click(event, messageModal) {
                    messageModal.hide();
                  },
                },
                {
                  text: lang("delete"),
                  click: function click(event, messageModal) {
                    App.Pages.Customers.remove(customerId), messageModal.hide();
                  },
                },
              ];
